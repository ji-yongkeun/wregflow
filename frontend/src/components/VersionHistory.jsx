...
export function VersionHistory({ fileId }) {
  ...
  const fetchVersions = async () => {
    setLoading(true)
    try {
      const response = await fetch(`http://localhost:8001/api/versions/file/${encodeURIComponent(fileId)}`)
  ...
  const handleCompare = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8001/api/versions/compare?version_id1=${selectedVersions[0]}&version_id2=${selectedVersions[1]}`,
        { method: 'POST' }
      )
      const data = await response.json()
